...
public class IntersectionOfTwoArrays2 {
    ...
    public IntersectionOfTwoArrays2(int[] nums1, int[] nums2, int[] expected) {
        this.nums1 = nums1;
        this.nums2 = nums2;
    ...
    public static Collection<Object[]> data() {
        return Arrays.asList(new Object[][] {
                ...
                {new int[] {1, 2, 2, 1}, new int[] {1, 2, 2, 1}, new int[] {1, 2, 2, 1}},
    ...
    public int[] intersection(int[] nums1, int[] nums2) {
        ...
        int[] alreadyComparedIndex = new int[largeNums.length];
        int alreadyComparedCount = 0;
        for(int i = 0; i < smallNums.length; i++) {
            for (int j = 0; j < largeNums.length; j++) {
                boolean alreadyCompared = false;
                for(int k = 0; k < alreadyComparedCount; k++) {
                    if(alreadyComparedIndex[k] == j)
                        alreadyCompared = true;
                }
                if(alreadyCompared)
                    continue;
                if(smallNums[i] == largeNums[j]) {
                    alreadyComparedIndex[alreadyComparedCount] = j;
                    alreadyComparedCount++;
